...
class HawkEye():
    ...
    def create_url(self, row, payload, kibana):
        ...
        payload['alias'] = None
        payload['query'] = row[self.fields['beacon']]
        url = kibana.generate_discover_url(**payload)
        row['raw_url'] = kibana.shorten_url(url)
        ...
        payload.update(query=event_id, filters=True, alias=json.dumps(alias))
        url = kibana.generate_discover_url(**payload)
        row['beacon_url'] = kibana.shorten_url(url)
        return row
    ...
    def find_beacons(self):
        ...
        beacon_fields = alias_list + \
            ['percent', 'interval', 'total', 'occurrences', 'id']
        ...
        beacon_df = beacon_df.sort_values(
            ['percent', 'total'], ascending=False).reset_index(drop=True)
        return beacon_df
    ...
    def analyze(self):
        ...
        if kbn['enable']:
            ...
            beacons = beacons.apply(lambda row: self.create_url(
                row, payload, kibana), axis=1)
        ...
        # Drop
        beacons.drop(['id'], axis=1, inplace=True)
